...
echo "********************************\n";
echo "*                              *\n";
echo "* OpenNemas requirements check *\n";
echo "*                              *\n";
echo "********************************\n\n";
...
check(is_writable(__DIR__.'/../tmp/cache'), sprintf('Checking that tmp/cache/ directory is writable'), 'Change the permissions of the tmp/cache/ directory so that the web server can write in it', true);
check(is_writable(__DIR__.'/../tmp/logs'),  sprintf('Checking that tmp/logs/ directory is writable'), 'Change the permissions of the tmp/logs/ directory so that the web server can write in it', true);
check(is_writable(__DIR__.'/../public/media'),  sprintf('Checking that public/media/ directory is writable'), 'Change the permissions of the public/media/ directory so that the web server can write in it', true);
...
check((extension_loaded('zlib')), 'Checking that the zlib extension is installed', 'Install and enable the zlib extension', true);
check((extension_loaded('ftp')), 'Checking that the ftp extension is installed', 'Install and enable the pspell extension', true);
check((extension_loaded('pspell')), 'Checking that the pspell extension is installed', 'Install and enable the pspell extension', true);
...
echo_title("Optional checks (Database connection)");

check(class_exists('PDO'), 'Checking that PDO is installed', 'Install PDO (mandatory for Doctrine)', false);
if (class_exists('PDO')) {
    $drivers = PDO::getAvailableDrivers();
    check(count($drivers), 'Checking that PDO has some drivers installed. Available drivers: '.implode(', ', $drivers), 'Install PDO drivers (mandatory for Doctrine)');
}
