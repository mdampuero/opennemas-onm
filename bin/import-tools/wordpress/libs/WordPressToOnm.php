...
/*************CATEGORIES MANAGE*************************/
    ...
    public function importArticles()
    {
        ...
        $_sql_where = ' `wp_term_relationships`.`term_taxonomy_id` IN ('.implode(', ', array_keys($this->originalCategories)).") ";
        $_limit = ''; /*'LIMIT 0,1'*/;
            ...
            while (!$rs->EOF) {
                ...
                if ($ih->elementIsImported($rs->fields['ID'], 'article') ) {
                    ...
                    $values = array(
                            ...
                            'metadata' => StringUtils::get_tags(ImportHelper::convertoUTF8($rs->fields['post_title'])),
                            'subtitle' => '',
                    ...
                    $data = $this->clearLabelsInBodyArticle($rs->fields['post_content']); //extract image html in body
                        $values =
                            array(
                                ...
                                'body' => $data['body'],
                                'metadata' => strip_tags(StringUtils::get_tags(ImportHelper::convertoUTF8($rs->fields['post_title']))),
                                'description' => strip_tags(substr($data['body'],0,150)),
        ...
        if (!$rs) {
            ...
            while (!$rs->EOF) {
                ...
                if ($ih->elementIsImported($rs->fields['ID'], 'image') ) {
                    ...
                    ////guid = http://www.correodiplomatico.com/wp-content/uploads/2011/09/Obama-Rusia.jpg
                    $local_file = str_replace('http://www.correodiplomatico.com/wp-content/uploads','',$rs->fields['guid']);
                  //  $local_file = str_replace(self::$originMedia['originUrl'],'',$rs->fields['guid']);
                    ...
                    $values = array(
                            ...
                            'frontpage' => 0,
                            'in_home' => 0,
                            'metadata' => StringUtils::get_tags(ImportHelper::convertoUTF8($rs->fields['post_title'].$rs->fields['post_excerpt'])),
                            'description' => ImportHelper::convertoUTF8(strip_tags(substr($rs->fields['post_excerpt'], 0, 150))),
                            'id' => 0,
                            'created' => $rs->fields['post_date'],
                            'starttime' => $rs->fields['post_date'],
                            'changed' => $rs->fields['post_modified'],
