...
function smarty_function_render_video($params, &$smarty)
{
    ...
    if (isset($video->video_content_replaced)) {
        ...
        $output = getInternalVideoOutput($params, $video);
...
function getInternalVideoOutput($params, $video)
{
    $rand   = rand();
    $output = '<a class="flashplayer" href="' . $params['base_url'] . DS
        . $video->video_url . '" style="display:block;width:'
        . $params['width'] . 'px;height:' . $params['height']
        . 'px;background: url(\'' . $video->thumb . '\')" id="flashplayer-'
        . $rand . '"></a>' . "\n";
