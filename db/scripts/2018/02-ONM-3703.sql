...
INSERT INTO users(username, email, name, type, fk_user_group, activated)
    SELECT email, email, CONCAT(`pc_users`.`name`, " ", `pc_users`.`firstname`, " ", `pc_users`.`lastname`), 1, 'pc_user', subscription
    FROM pc_users
    WHERE NOT EXISTS (
        SELECT id
        FROM `users`
        WHERE `users`.email = `pc_users`.email
    );

INSERT INTO user_user_group(user_id, user_group_id, status)
    SELECT id, @group, activated
    FROM users
    WHERE fk_user_group='pc_user' AND email IN (SELECT email FROM `pc_users`);

INSERT INTO user_user_group(user_id, user_group_id, status)
    SELECT id, @group, activated
    FROM users
    JOIN pc_users ON pc_users.email = users.email
    WHERE id NOT IN (
        SELECT user_id
