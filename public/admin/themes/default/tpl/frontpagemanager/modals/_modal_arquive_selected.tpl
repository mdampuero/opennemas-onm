<div class="modal hide fade" id="modal-batch-arquive">
    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal-batch-arquive" aria-hidden="true">×</button>
      <h3>{t}Arquive these elements{/t}</h3>
    </div>
    <div class="modal-body">
        <p>
            {t escape=off}Are you sure that do you want to arquive the selected elements from this frontpage?{/t}<br/>
            {t escape=off}This will remove selected elements from <strong>ALL FRONTPAGES</strong> but they will remain available for read.{/t}
        </p>
    </div>
    <div class="modal-footer">
        <a class="btn btn-primary yes" href="#">{t}Arquive{/t}</a>
        <a class="btn no" href="#">{t}Keep{/t}</a>
    </div>
</div>