<table id='tabla{$aux}' name='tabla{$aux}' value="{$item->pk_widget}" data="{$item->content_type}" width="100%" class="tabla">
    <tr class="row1{schedule_class item=$item}" style="cursor:pointer;">
        ...
        {if $category neq 'home'}
            <td class="un_no_view" style="width:20px;" align="center">
                <div class="inhome" style="display:inline;">
                    {if $item->in_home == 1}
                    <a href="controllers/widget/widget.php?id={$item->id}&amp;action=inhome_status&amp;status=0&amp;category={$category}"  title="No sugerir en home"  alt="No sugerir en home">
                    ...
                    {else}
                        <a href="controllers/widget/widget.php?id={$item->id}&amp;action=inhome_status&amp;status=2&amp;category={$category}" class="go_home" title="Sugerir en home" alt="Sugerir en home"></a>
                    {/if}
                </div>
            </td>
        {/if}
        ...
        <td  align="center"  class="un_width" style="width:20px;">
                <a  href="controllers/widget/widget.php?action=archive&id={$item->pk_widget}&category={$category}"  title="Archivar">
        ...
        {else}
             <td  align="center"  class="un_width" style="width:25px;">
                <a href="controllers/widget/widget.php?action=unpublish&id={$item->pk_widget}" class="no_home" title="Quitar de home" alt="Quitar de home" ></a>
            </td>
        {/if}
    </tr>
</table>
