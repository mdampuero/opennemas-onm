...
updateLink = function() {
    ...
    if (name) {jQuery('#'+id).attr('title', name);}
    if (link) { jQuery('#'+id).attr('link', link); }
...
createActions = function(id) {
    jQuery('#'+id).append(
        '<div class="div-actions" style="margin-top:10px; width: 60px;">'+
        '<a onclick="editLink(\''+id+'\');">Edit </a>  '+
        '| <a onclick="deleteLink(\''+id+'\');">Delete </a> </div>');
};
