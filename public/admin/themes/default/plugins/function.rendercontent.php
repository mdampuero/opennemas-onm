...
function smarty_function_rendercontent($params, &$smarty) {
    $output = '';
    ...
    foreach($items as $i => $item) {
        if( $item->{$property} == $placeholder ) {
            $content_type = ContentManager::getContentTypeNameFromId($item->content_type);
            $tpl = "frontpage/content_type_row/$content_type.tpl";
