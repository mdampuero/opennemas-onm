...
switch ($action) {
        ...
        */
        if (!empty($stringSearch)) {
            ...
            $arrayResults   = $objSearch->searchContentsSelectMerge(
                ...
                contents_categories.pk_fk_content_category as category,
                content_types.title as type, contents.available,
    ...
    case 'search_paging':
        ...
        $arrayResults   = $objSearch->searchContentsSelectMerge(
            ...
            contents.available, contents.content_status,
            contents.in_litter, content_types.name as content_type",
            $szTags,
            $szCheckedTypes,
 ...
 * Name: checkTypes
 *
 * Description: Parsea el $_REQUEST y obtiene un string con los tipos de
 * contenidos enviados a la página.
 *
 * Input:   $void
 *
 * Output: cadena de texto con los nombre de los tipos de contenidos
 * separados por comas.
**/
        ...
        if (mod::moduleExists(strtoupper($aType['name']).'_MANAGER')) {
            if (mod::isActivated(strtoupper($aType['name']).'_MANAGER')) {
                if (isset($_REQUEST[$aType[1]])) {
                    $szTypes   .= $aType[1] . ", ";
                    $htmlCheck .= '<input id="'.$aType[1] .'" name="' .
                        $aType[1] .
                        '"  type="checkbox" valign="center" checked="true"/>'.
                        $aType['title'];
                } else {
                    $htmlCheck .= '<input id="'. $aType[1].'" name="'.
                        $aType[1].'"  type="checkbox" valign="center"/>'.
                        $aType['title'];
                }
            }
        }
 ...
 * Name: paginateLink
 *
 * Description: Crea los link clicables con tres paginas para seleccionar
 * y un primera y última.
 *
 * Input:   $Pager.......: (object) Paginador de la libreria externa.
 *          $szSearchString..: (strings) Metadatos a buscar en la base de datos.
 *          $arrayCheckedTypes..: (array) Array con los tipos de datos en los
 *          cuales buscaremos.
 *
    ...
    if ($Pager->_totalPages>1) {
        ...
        if ($Pager->_currentPage != 1) {
            $szPages .= '<a style="cursor:pointer;" href="#" '.
                'onclick="paginate_search(\'search_paging\', 1, \''.
                $szSearchString.'\', \'';
        ...
        for ($iIndex=$Pager->_currentPage-1;
            $iIndex<=$Pager->_currentPage+1 &&
            $iIndex <= $Pager->_totalPages;$iIndex++) {
            if ($Pager->_currentPage == 1) {
