...
if (isset($configurations['size_file']) && !empty($configurations['size_file'])) {
    $sizeFile = $configurations['size_file'];
} else {
    $sizeFile = 5000000;
}
...
switch($action) {
    ...
    case 'list':
        ...
        if ($category == 'widget') {
            $specials = $cm->find_all('Special', 'in_home =1 AND available =1',
        ...
        if (empty($id)) {
            ...
            if (!Acl::isAdmin()
                ...
                && $special->fk_user != $_SESSION['userid']) {
                    m::add(_("You can't modify this special because ".
                                    "you don't have enought privileges.") );
            }
            $special->update($_POST);

        }
        Application::forward($_SERVER['SCRIPT_NAME'].'?action=read&id='.
                            $special->id.'&category='.$category.'&page='.$page);
    ...
    case 'change_inHome':
        ...
        if ($special->available == 1) {
            ...
            m::add(_("This special is not published so you can't define ".
                        "it as widget home content.") );
        }
        Application::forward($_SERVER['SCRIPT_NAME'].'?action=list&category='.
                            $category);
    ...
    case 'change_status':
        ...
        if (isset($_GET['desde']) && $_GET['desde']=='search') {
            Application::forward('search.php?action=search&stringSearch='.
                            $_GET['stringSearch'].'&page='.$_GET['page']);
        }
        Application::forward($_SERVER['SCRIPT_NAME'].'?action=list&category='.
                            $category);
    break;
    ...
    case 'change_article_page':
        $articles = $cm->find_by_category('Article', $category,
                                ' 1=1 ', 'ORDER BY created DESC LIMIT 0,100');
        $params   = $category;
        $articles = $cm->paginate_num_js($articles, 16, 1,
                            'changeSpecials', $params);
    ...
    case 'save_config':
        ...
        Application::forward($_SERVER['SCRIPT_NAME'] . '?'.
                StringUtils::toHttpParams($httpParams));
