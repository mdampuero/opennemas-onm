...
if (!isset($action)) {
    $action = filter_input(INPUT_GET, 'action', FILTER_SANITIZE_STRING,
                array('options' => array('default' => 'addContents')));
}
...
switch($action) {
    ...
    case 'preview':
        ...
        if (array_key_exists('newsletterHtml', $_SESSION) &&
            ...
            $htmlContent = htmlspecialchars_decode($_SESSION['newsletterHtml'],
                                                    ENT_QUOTES);
        } else {
    ...
    case 'send':
        ...
        foreach ($recipients as $mailbox) {
            ...
            $emailHtmlContent = str_replace('###DESTINATARIO###',
                                    $mailbox->name, $htmlContent);

            if ($nManager->sendToUser($mailbox, $emailHtmlContent, $params)) {
                $htmlFinal .= '<tr><td width=50% align=right>'.
                '<strong class="ok">OK</strong>&nbsp;&nbsp;</td><td>'.
                $mailbox->name . ' &lt;' . $mailbox->email . '&gt;</td></tr>';
            } else {
                $htmlFinal .= '<tr><td width=50% >'.
                '<strong class="failed">FAILED</strong>&nbsp;&nbsp;</td><td>'.
                $mailbox->name . ' &lt;' . $mailbox->email. '&gt;</td></tr>';
            }
        }
