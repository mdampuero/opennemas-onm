...
class Author {
	...
	* @param string $_order_by, the ORDER BY sql part to sort authors with
	* @return mixed, array of all matched authors
	*/
    static public function list_authors($filter=NULL, $_order_by='ORDER BY 1') {

        $items = array();
        $_where = '1=1';
        if( !is_null($filter) ) { $_where = $filter; }

        $sql = 	'SELECT `authors`.`pk_author`, `authors`.`name` , `authors`.`gender` ,'
				.' `authors`.`politics` , `authors`.`date_nac` , `authors`.`fk_user` ,'
				.' `authors`.`condition` FROM `authors` WHERE '.$_where.' '.$_order_by;
