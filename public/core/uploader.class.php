...
class Uploader {
    ...
    function upload() {
        if( !is_null($this->name) && !is_null($this->path)) {
            ...
            if (move_uploaded_file($_FILES[ $this->name ]['tmp_name'], $uploadfile)) {
                ...
                // Disparar evento despu�s de Upload
    ...
    */
    ...
    /**
     * Reescalar las imagenes a los valores por defecto
    */
    function scale($width=DEFAULT_WIDTH, $height=DEFAULT_HEIGHT) {
        ...
        if(preg_match('/(\.png|\.jpg|\.jpeg|\.gif|\.bmp)$/i', $this->basename)) {
            ...
            // Invertir si es m�s alto que ancho
    ...
    */
    function post_video() {
        ...
        if(preg_match('/(\.avi|\.mpg|\.mpeg|\.3gp|\.flv)$/i', $this->basename)) {
            // Incluir las clases para subir v�deos a youtube
            require_once(SITE_LIBS_PATH.'phptube.php');
    ...
    */
    ...
    /**
     * Establece un entorno de ejecuci�n de PHP para poder subir ficheros
    */
