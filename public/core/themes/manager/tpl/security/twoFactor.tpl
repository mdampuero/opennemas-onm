...
{block name="leftFilters"}
  ...
  <li class="hidden-xs m-r-10 ng-cloak quicklinks"
      ng-init="twoFactorStates = [ { name: '{t}Any{/t}', value: null }, { name: '{t}Enabled{/t}', value: '1' }, { name: '{t}Disabled{/t}', value: '0' } ]">
    <ui-select name="two_factor_enabled" theme="select2" ng-model="criteria.two_factor_enabled">
      <ui-select-match>
        <strong>{t}2FA{/t}:</strong> [% $select.selected.name %]
      </ui-select-match>
      <ui-select-choices repeat="option.value as option in twoFactorStates | filter: $select.search">
