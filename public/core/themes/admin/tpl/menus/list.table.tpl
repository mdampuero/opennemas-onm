...
<td class="v-align-middle" ng-if="isColumnEnabled('name')">
  ...
  {acl isAllowed="MENU_UPDATE"}
    ...
    <translator item="data.items[$index]" keys="keys" link="[% routing.generate('backend_menu_show', { id: getItemId(item) }) %]" ng-class="{ 'dropup': $index >= items.length - 1 }" ng-if="data.extra.locale.multilanguage && data.extra.locale.available" options="data.extra.locale" text="{t}Edit{/t}"></translator>
  {/acl}
