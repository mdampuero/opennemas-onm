<amp-analytics type="googleanalytics" config="{$configfile}" data-credentials="include">
    <script type="application/json">
        {
            "vars": {
              "GA4_MEASUREMENT_ID": "{$ga4Id}",
              "GA4_ENDPOINT_HOSTNAME": "www.google-analytics.com",
              "DEFAULT_PAGEVIEW_ENABLED": true,
              "GOOGLE_CONSENT_ENABLED": false,
              "WEBVITALS_TRACKING": false,
              "PERFORMANCE_TIMING_TRACKING": false,
              "SEND_DOUBLECLICK_BEACON": false
            },
            "extraUrlParams": {
                "event__str_sponsored": "0",
                "event__str_ads_enabled": "1",
                "event__str_layout": "{$layout}",
                "event__str_categoria": "{get_category_slug($content)}",
                "event__str_subcategoria": "sinsc",
                "event__str_terciariacategoria": "sinct",
                "event__str_createdby": "{get_author_id($content)}",
                "event__str_lastmodify": "{$lastAuthorId}",
                "event__str_firma": "{get_author_name($content)}",
                "event__str_fechapublicacion": "{format_date date=$content->starttime format="YYYYMMdd" type="custom"}",
                "event__str_fechadeactualizacion": "{format_date date=$content->changed format="YYYYMMdd" type="custom"}",
                "event__str_keywords": "{$tagNames}",
                "event__str_noticia_id": "{get_id($content)}",
                "event__str_mediatype": "{$mediaType}",
                "event__str_seotag": "{$tagSlugs}",
                "event__str_platform": "amp",
                "event__str_ga_page_host": "{literal}${ampdocHostname}{/literal}",
                "event__str_user_id": "",
                "event__str_user_logged": "0",
                "event__str_gaid": "",
                "event__str_su": ""
            }
        }
    </script>
</amp-analytics>
