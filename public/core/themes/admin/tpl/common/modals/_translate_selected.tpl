<div class="modal-body">
  ...
  <div ng-hide="template.config.translators.length > 0" class="text-center m-t-50 p-t-30">
    ...
    <p class="m-b-50">{t escape=off}Please go to the <a href="{url name=backend_settings_language}">Settings page</a> and configure your <br> translators for all the languages.{/t}</p>
  </div>
  ...
  <div ng-show="template.translation_done">
      <div class="text-center m-t-50 p-t-30">
        ...
        <h4>{t 1="[% template.selected.length %]" 2="[% template.config.locales[template.config.translateTo] %]"}%1 contents propertly translated into "%2".{/t}</h4>
      </div>
