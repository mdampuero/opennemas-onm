<div class="grid-collapse-title ng-cloak pointer" ng-click="expanded.{$field} = !expanded.{$field}">
  <i class="fa fa-external-link m-r-10"></i>{$title}
  <i class="fa fa-chevron-right pull-right m-t-5" ng-class="{ 'fa-rotate-90': expanded.{$field} }"></i>
  <span ng-show="!expanded.{$field} && item.pk_content > 0" class="badge badge-default m-r-10 m-t-2 ng-cloak pull-right text-uppercase text-bold" target="_blank">
    <strong>{$title}</strong>
  </span>
</div>
