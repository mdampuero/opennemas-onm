<ui-select class="{$class}" name="author" ng-model="{$ngModel}" {if $required}required{/if} theme="select2">
  ...
  <ui-select-choices repeat="item.id as item in toArray(addEmptyValue(data.extra.authors, 'id'{if $select}, 'name', '{t}Select an author{/t}…'{/if})) | filter: { name: $select.search }">
