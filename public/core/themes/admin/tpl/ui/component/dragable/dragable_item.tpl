<li ng-repeat="item in {$iData} track by item.link_name+item.referenceId" ui-tree-node class="ng-scope angular-ui-tree-node"  {if $iFilter}ng-show="visible(item)"{/if}>
