          ...
          <tbody>
            ...
            <tr ng-if="items.length >= 0" ng-repeat="item in items" ng-class="{ row_selected: isSelected(item.id) }">
              ...
              <td class="text-center">
                <i class="fa text-[% item.style %]" ng-class="{ 'fa-database': item.type === 'media', 'fa-envelope': item.type === 'email', 'fa-support': item.type === 'help', 'fa-info': item.type === 'info', 'fa-users': item.type === 'users', }" tooltip="[% extra.types[item.type].name %]"></i>
              </td>
              <td class="text-center">
                [% item.start %]
              </td>
