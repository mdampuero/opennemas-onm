    ...
    */
    ...
    if (isset ($banner)) { 
        if ($banner->with_script == 1) {
            ...
            $output.= '<iframe src="/publicidade/get/' . $banner->pk_content . '.html" ' . 'scrolling="no" frameborder="0" width="' . $width . '" height="' . $height . '" ' . 'marginwidth="0" marginheight="0" rel="nofollow">Publicidad Retrincos.com</iframe>';
                ...
                if (!$overlap && !$banner->overlap) {
                    ...
                    if (!$isBastardIE) {
                        $output.= '<div style="position: relative; width: ' . $width . 'px; height: ' . $height . 'px;">
                            <div style="left:0px;top:0px;cursor:pointer;background-color:transparent;position:absolute;z-index:100;width:' . $width . 'px;height:' . $height . 'px;"
                                onclick="javascript:window.open(\'/publicidade/' . $banner->pk_advertisement . '.html\', \'_blank\');return false;"></div>';
                    } else {
                        $output.= '<div style="position: relative; width: ' . $width . 'px; height: ' . $height . 'px;">
                            <div style="left:0px;top:0px;cursor:pointer;background-color:#FFF;filter:alpha(opacity=0);position:absolute;z-index:100;width:' . $width . 'px;height:' . $height . 'px;"
                                onclick="javascript:window.open(\'/publicidade/' . $banner->pk_advertisement . '.html\', \'_blank\');return false;"></div>';
                    }
