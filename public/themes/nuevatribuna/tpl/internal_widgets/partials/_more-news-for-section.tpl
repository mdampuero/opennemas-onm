    ...
    <div class="more-news-section span-8 {if ($last)}last{/if}">
        ...
        <ul class="more-news-section-links">
            {foreach name=t key=c item=sub from=$titulares_cat[$index]}
                ...
                {if $smarty.foreach.t.first}
                    <li class="first">
                        <a href="{$smarty.const.SITE_URL}{generate_uri
                                                                    ...
                                                                    title=$sub.title}" title="{$sub.title|clearslash|escape:"html"|clean_for_html_attributes}">{$sub.title|clearslash}</a>
                    </li>
                ...
                {else}
                    <li>
                        <a href="{$smarty.const.SITE_URL}{generate_uri content_type='article' id=$sub.id date=$sub.created category_name=$category_hdata.name title=$sub.title}"
