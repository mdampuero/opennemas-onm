...
<div class="onm-new {$cssclass}">
    ...
    {if $category_name eq 'home'}
        <div class="nw-category-name {$item->category_name}">{$item->category_title|upper|htmlentities} <span>&nbsp;</span></div>
    {/if}
    <h3 class="nw-title"><a href="{$item->permalink|clearslash}" title="{$item->title|htmlentities}">{$item->title|htmlentities}</a></h3>
