        ...
        <ul>
            ...
            <li {if $smarty.section.a.last}class="last"{/if}>
                {if $latestOpinions[a]->name neq ""}
                    <strong><a href="{$smarty.const.SITE_URL}{generate_uri content_type="opinion_author_frontpage" title=$latestOpinions[a]->name id=$latestOpinions[a]->pk_author}">{$latestOpinions[a]->name}</a></strong>
                    <br>
                {/if}
