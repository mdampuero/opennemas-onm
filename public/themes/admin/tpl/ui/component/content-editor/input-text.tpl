<div class="form-group">
  ...
  <div class="controls">
    {if $counter}<div class="input-group">{/if}
      <input type="text" id="{$field}" name="{$field}" ng-model="item.{$field}" {if $required}required{/if} class="form-control" uib-tooltip="[% data.item.title[data.extra.locale.default] %]" tooltip-enable="data.extra.locale.default !== config.locale.selected"/>
      {if $counter}
