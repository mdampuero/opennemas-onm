...
{block name="itemActions"}
  {acl isAllowed="POLL_UPDATE"}
    ...
    <translator item="data.items[$index]" keys="data.extra.keys" link="[% routing.generate('backend_poll_show', { id: getItemId(item) }) %]" ng-class="{ 'dropup': $index >= data.total - 1 }" ng-if="data.extra.locale.multilanguage && data.extra.locale.available" options="data.extra.locale" text="{t}Edit{/t}"></translator>
  {/acl}
