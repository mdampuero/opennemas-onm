...
{block name="ngInit"}
  ng-controller="UserListCtrl" ng-init="init();backup.master = {if $app.security->hasPermission('MASTER')}true{else}false{/if};backup.id = {$app.user->id}"
{/block}
...
{block name="list"}
  {include file="user/list.table.tpl"}
{/block}
