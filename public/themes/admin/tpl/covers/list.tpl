    ...
    <table class="table table-hover table-condensed" ng-if="!loading">
        ...
        <tbody {if $category == 'widget'}ui-sortable ng-model="shvs.contents"{/if}>
            ...
            <tr ng-if="shvs.contents.length > 0" ng-repeat="content in shvs.contents" ng-class="{ row_selected: isSelected(content.id) }" data-id="[% content.id %]">
                ...
                {acl isAllowed="KIOSKO_HOME"}
                <td class="center">
                    <button class="btn-link" ng-class="{ 'loading': content.home_loading == 1, 'go-home': content.in_home == 1, 'no-home': content.in_home == 0 }" ng-click="updateItem($index, content.id, 'backend_ws_content_toggle_in_home', 'in_home', content.in_home != 1 ? 1 : 0, 'home_loading')" type="button">
                    </button>
                </td>
                {/acl}
            </tr>
