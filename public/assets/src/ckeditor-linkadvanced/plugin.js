...
( function() {
	CKEDITOR.plugins.add( 'linkadvanced', {
		requires: 'dialog,fakeobjects',
		icons: 'anchor,anchor-rtl,linkadvanced,unlink', // %REMOVE_LINE_CORE%
		hidpi: true, // %REMOVE_LINE_CORE%
		onLoad: function() {
			// Add the CSS styles for anchor placeholders.
			var iconPath = CKEDITOR.getUrl( '/assets/src/ckeditor-linkadvanced/icons/hidpi/anchor.png' ),
		...
		init: function( editor ) {
			...
			if ( CKEDITOR.dialog.isTabEnabled( editor, 'linkadvanced', 'advanced' ) )
				allowed = allowed.replace( ']', ',accesskey,charset,dir,id,lang,name,rel,tabindex,title,type]{*}(*)' );
			if ( CKEDITOR.dialog.isTabEnabled( editor, 'linkadvanced', 'target' ) )
				allowed = allowed.replace( ']', ',target,onclick]' );

			// Add the link and unlink buttons.
			editor.addCommand( 'linkadvanced', new CKEDITOR.dialogCommand( 'linkadvanced', {
			...
			editor.setKeystroke( CKEDITOR.CTRL + 76 /*L*/, 'linkadvanced' );

			if ( editor.ui.addButton ) {
				editor.ui.addButton( 'linkadvanced', {
					label: editor.lang.linkadvanced.toolbar,
					command: 'linkadvanced',
					toolbar: 'links,10',
          icon: '/assets/src/ckeditor-linkadvanced/icons/linkadvanced.png'
				} );
				editor.ui.addButton( 'Unlink', {
					label: editor.lang.linkadvanced.unlink,
					command: 'unlink',
					toolbar: 'links,20'
				} );
				editor.ui.addButton( 'Anchor', {
					label: editor.lang.linkadvanced.anchor.toolbar,
					command: 'anchor',
					toolbar: 'links,30'
				} );
			}

			CKEDITOR.dialog.add( 'linkadvanced', '/assets/src/ckeditor-linkadvanced/dialogs/link.js' );
			CKEDITOR.dialog.add( 'anchor', '/assets/src/ckeditor-linkadvanced/dialogs/anchor.js' );

			editor.on( 'doubleclick', function( evt ) {
				var element = CKEDITOR.plugins.linkadvanced.getSelectedLink( editor ) || evt.data.element;

				if ( !element.isReadOnly() ) {
					if ( element.is( 'a' ) ) {
						evt.data.dialog = ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) ? 'anchor' : 'linkadvanced';

						// Pass the link to be selected along with event data.
						evt.data.linkadvanced = element;
					} else if ( CKEDITOR.plugins.linkadvanced.tryRestoreFakeAnchor( editor, element ) ) {
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( evt.data.dialog in { linkadvanced: 1, anchor: 1 } && evt.data.linkadvanced )
					editor.getSelection().selectElement( evt.data.linkadvanced );
			}, null, null, 20 );
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					anchor: {
						label: editor.lang.linkadvanced.anchor.menu,
						command: 'anchor',
						group: 'anchor',
						order: 1
					},
					...
					removeAnchor: {
						label: editor.lang.linkadvanced.anchor.remove,
					...
					linkadvanced: {
						label: editor.lang.linkadvanced.menu,
						command: 'linkadvanced',
						group: 'linkadvanced',
						order: 1
					},
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element ) {
					...
					var anchor = CKEDITOR.plugins.linkadvanced.tryRestoreFakeAnchor( editor, element );

					if ( !anchor && !( anchor = CKEDITOR.plugins.linkadvanced.getSelectedLink( editor ) ) )
						return null;

					var menu = {};

					if ( anchor.getAttribute( 'href' ) && anchor.getChildCount() )
						menu = { linkadvanced: CKEDITOR.TRISTATE_OFF, unlink: CKEDITOR.TRISTATE_OFF };
		...
		afterInit: function( editor ) {
			...
			if ( pathFilters ) {
				pathFilters.push( function( element, name ) {
					if ( name == 'a' ) {
						if ( CKEDITOR.plugins.linkadvanced.tryRestoreFakeAnchor( editor, element ) || ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) )
							return 'anchor';
					}
				} );
			}
	...
	function protectEmailLinkAsFunction( editor, email ) {
		var plugin = editor.plugins.linkadvanced,
	...
	CKEDITOR.plugins.linkadvanced = {
		...
		parseLinkAttributes: function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				compiledProtectionFunction = editor.plugins.linkadvanced.compiledProtectionFunction,
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function( editor ) {
			var sel = editor.getSelection(),
				...
				anchor;
			if ( sel && ( anchor = sel.getSelectedElement() ) && ( !anchor.getChildCount() ? CKEDITOR.plugins.linkadvanced.tryRestoreFakeAnchor( editor, anchor ) : anchor.is( 'a' ) ) )
				anchor.remove( 1 );
			else {
				if ( ( anchor = CKEDITOR.plugins.linkadvanced.getSelectedLink( editor ) ) ) {
