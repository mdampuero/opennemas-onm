...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.linkadvanced.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			var sel = editor.getSelection(),
				...
				linkElement = fakeSelected ?
					CKEDITOR.plugins.linkadvanced.tryRestoreFakeAnchor( editor, fullySelected ) :
					CKEDITOR.plugins.linkadvanced.getSelectedLink( editor );
		...
		contents: [ {
			id: 'info',
			label: editor.lang.linkadvanced.anchor.title,
			accessKey: 'I',
			elements: [ {
				type: 'text',
				id: 'txtName',
				label: editor.lang.linkadvanced.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.linkadvanced.anchor.errorName ); // jshint ignore:line
						return false;
					}
					return true;
				}
