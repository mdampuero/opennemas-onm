...
class XmlFormat implements iFormat
{
    ...
    public static $text_node_name='text';
    ...
    {
        return $this->toXML(object_to_array($data, false), self::$root_name, $human_readable);
    }
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $key => $value) {
                ...
                if (is_array($value)) {
                    ...
                    if (in_array($key, self::$attribute_names)) {
                        $xml->addAttribute($key, $value);
                    } elseif ($key==self::$text_node_name) {
                        $xml->{0} = $value;
                    } else {
