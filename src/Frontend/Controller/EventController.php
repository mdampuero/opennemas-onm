...
class EventController extends FrontendController
{
    ...
    {
        $categoryId = !empty($params['o_category'])
        ...
        $params['isSuggested'] = isset($params['content'])
            && isset($params['content']->pk_content)
            && count(array_filter($params['suggested'], function ($suggested) use ($params) {
                return $params['content']->pk_content == $suggested->pk_content;
            })) > 0;

        if ($params['isSuggested'] && !empty($categoryId)) {
            $params['x-tags'] .= sprintf(',last-suggested-%d', $categoryId);
        }
    }
