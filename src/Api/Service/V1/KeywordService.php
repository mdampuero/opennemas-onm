...
class KeywordService extends OrmService
{
    ...
    public function replaceTerms($text, $keywords)
    {
        ...
        $text = $this->removeAutoGeneratedLinks($text);
    ...
    {
        return sprintf(
            '<a href="%s" target="_blank" data-generated="auto">%s</a>',
        ...
        $result = preg_replace_callback(
            ...
            function ($matches) use ($placeholder) {
                if (!empty($matches[1])) {
                    ...
                    $this->replacedKeywords[$placeholder] = true;
                    return $placeholder;
                } else {
                    return $matches[0];
                }
    ...
    /**
     * Removes all <a> tags with data-generated="auto" while keeping the inner text.
     *
     * @param string $text The HTML content to process.
     * @return string The processed text with the specified links removed but inner text retained.
     */
    private function removeAutoGeneratedLinks($text)
    {
        return preg_replace_callback(
            '@<a\b[^>]*\bdata-generated="auto"[^>]*>(.*?)<\/a>@',
            function ($matches) {
                return $matches[1]; // Return the inner text of the <a> tag
            },
            $text
        );
    }
