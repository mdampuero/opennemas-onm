<div class="ColumnHome2">
    <div class="ColumnHome2-preHeader">{$item->subtitle|clearslash}</div>
    <h2 style="font-size:20px;"><a href="{$item->permalink|clearslash}">{$item->title|clearslash}</a></h2>
        ...
        {foreach from=$photos key=myId item=i}
            {if !strcmp(strval($myId), strval($item->id)) and !empty($i)}
                <div class="ColumnHome2_photo">
                    <div class="ColumnHome2_photo2"><img height="105" src="{$MEDIA_IMG_PATH_WEB}{$i}" alt="{$item->img1_footer|clearslash}" /></div>
