...
class ContentManager {    
    ...
    function ContentManager($content_type=NULL) {
 ...
 function find_all($content_type, $filter=NULL, $_order_by='ORDER BY 1', $fields='*') {
        $this->init($content_type);
        $items = array();
    ...
    function count($content_type, $filter=NULL, $pk_fk_content_category=NULL) {
        ...
        if( intval($pk_fk_content_category) != NULL) {
            $sql = 'SELECT COUNT(contents.pk_content) FROM `contents_categories`, `contents`, '.$this->table.'  ' .
                ' WHERE '.$_where.' AND `contents_categories`.`pk_fk_content_category`='.$pk_fk_content_category.'  AND pk_content=`'.$this->table.'`.`pk_'.strtolower($content_type).'` AND  `contents_categories`.`pk_fk_content` = `contents`.`pk_content` ';
        } else {       
...
//        die();
    ...
    function find_by_category($content_type, $pk_fk_content_category, $filter=NULL, $_order_by='ORDER BY 1')
    {
        $this->init($content_type);
        ...
        if( intval($pk_fk_content_category) > 0 ) {
            $sql = 'SELECT * FROM contents_categories, contents, '.$this->table.'  ' .
                'WHERE '.$_where.' AND `contents_categories`.`pk_fk_content_category`='.$pk_fk_content_category.'  AND `contents`.`pk_content`=`'.$this->table.'`.`pk_'.strtolower($content_type).'` AND  `contents_categories`.`pk_fk_content` = `contents`.`pk_content` '.$_order_by;
        } else {
            return( $items );
        }
    ...
    function find_by_category_name($content_type, $category_name, $filter=NULL, $_order_by='ORDER BY 1')
    {
    ...
    function find_inSubcategory_by_categoryName($content_type, $category_name, $filter=NULL, $_order_by='ORDER BY 1')
    {
        $this->init($content_type);
        $items = array();
...
// Assing values to new object call Headline
    function find_category_headline($pk_fk_content_category, $filter=NULL, $_order_by='ORDER BY 1')
    {
        $_where = 'in_litter=0';
        if( !is_null($filter) ) {
            if( preg_match('/in_litter=1/i', $filter) ) { //se busca desde la litter.php
         ...
         $sql = 'SELECT contents.pk_content, contents.title, contents.permalink, contents.created, contents.changed, contents.starttime, contents.endtime  FROM contents_categories, contents ' .
                 'WHERE contents.fk_content_type=1 and '.$_where.' AND pk_fk_content_category=\''.$pk_fk_content_category.'\'  AND  pk_fk_content = pk_content '.$_order_by;
    ...
    function find_headlines(/*$filter=NULL, $_order_by='ORDER BY 1'*/)
    {
  
          $sql='SELECT `contents`.`title` , `contents`.`created` ,  `contents`.`permalink` , `contents`.`starttime` ,  `contents`.`endtime` , `contents_categories`.`pk_fk_content_category` AS `category_id` 
                FROM `contents`
    ...
    //this function returns title,catName and permalinks of last headlines from Subcategories of a given category
    function find_listAuthors($filter=NULL, $_order_by='ORDER BY 1')
    {
        $this->init($content_type);
        $items = array();
        $_where = '1=1  AND in_litter=0';
        ...
        //necesita el as id para paginacion
         ...
         $sql= 'SELECT contents.pk_content, opinions.pk_opinion as id, authors.name,authors.pk_author,authors.condition,contents.title,author_imgs.path_img,contents.permalink,opinions.type_opinion,opinions.body,contents.created,contents.starttime,contents.endtime '.
    ...
    function find_listAuthorsEditorial($filter=NULL, $_order_by='ORDER BY 1')
    {
        $this->init($content_type);
        $items = array();
        $_where = '1=1  AND in_litter=0';
        ...
        if( !is_null($filter) )
        {
         ...
         $sql= 'SELECT authors.name, opinions.pk_opinion as id, contents.title,contents.permalink,opinions.type_opinion,opinions.body,contents.created'.
            ' FROM contents, opinions'.
            ' LEFT JOIN authors ON (authors.pk_author=opinions.fk_author)'.
            ' where `contents`.`fk_content_type`=4 and opinions.type_opinion=1'.
     ...
     */
    function create_paginate($total_items, $num_pages, $delta, $funcion='NULL', $params='NULL'){
        if(!isset($num_pages)){
            $num_pages = 5;
        }
        ...
        if($funcion == 'URL'){
            ...
            $path = SITE_URL.$params;
              if($params=='/seccion/opinion'){  $page='pageop';} //En listado de opinion, hay dos pages. List autors y list opinions.
    ...
    //author.php,¿pconecta?,mediamanager??
...
//FIXME: unificar todos los paginates
 ...
 //Admin  advertisement.php, advertisement_images.php, opinion.php, preview_content.php
    ...
    //Paginate para contents de num_pages
    function paginate_num_js($items, $num_pages, $delta, $funcion,$params='NULL') {
    	if(!isset($num_pages)){
            $num_pages = 20;
        }
  ...
  //FIXME: unificar todos los paginates
    function paginate_array_num_js($items, $num_pages, $delta, $funcion, $params='NULL') {
        $_items = array();
  ...
  //admin article.php, article_change_videos.php
    function makePagesLinkjs($Pager, $funcion, $params) {
        $szPages=null;
        if($Pager->_totalPages>1)
        {
            ...
            if($Pager->_currentPage != $Pager->_lastPageText)
                $szPages .= '... <a style="cursor:pointer;" onClick="'.$funcion.'('.$params.','.$Pager->_lastPageText.');">Última </a>';
            $szPages .= "</p> ";
        }
        return $szPages;
    }
...
//admin article.php, article_change_images.php
  function makePagesLink($Pager, $category, $action, $metadatas) {
        $szPages=null;
        if($Pager->_totalPages>1)
        {
            $szPages = '<p align="center">';
            if ($Pager->_currentPage != 1)
                $szPages .= '<a style="cursor:pointer;" onClick="get_images('.$category.',1, \''.$action.'\', \''.$metadatas.'\');">Primera</a> ... | ';
            ...
            for($iIndex=$Pager->_currentPage-2; $iIndex<=$Pager->_currentPage+2 && $iIndex <= $Pager->_totalPages;$iIndex++)
            {
                if($Pager->_currentPage == 1)
                {
                    if(($iIndex+2) > $Pager->_totalPages)
                        break;
                    $szPages .= '<a style="cursor:pointer;" onClick="get_images('.$category.','.($iIndex+2).', 	\''.$action.'\', \''.$metadatas.'\');">';
                    if($Pager->_currentPage == ($iIndex+2))
                        $szPages .= '<b>' . ($iIndex+2) . '</b></a> | ';
                    else
                        $szPages .= ($iIndex+2) . '</a> | ';
                }elseif($Pager->_currentPage == 2)
                {
                    if(($iIndex+1) > $Pager->_totalPages)
                        break;
                    $szPages .= '<a style="cursor:pointer;" onClick="get_images('.$category.','.($iIndex+1).', 	\''.$action.'\', \''.$metadatas.'\');">';
                    if($Pager->_currentPage == ($iIndex+1))
                        $szPages .= '<b>' . ($iIndex+1) . '</b></a> | ';
                    else
                        $szPages .= ($iIndex+1) . '</a> | ';
                ...
                {
                    $szPages .= '<a style="cursor:pointer;" onClick="get_images('.$category.','.$iIndex.', \''.$action.'\', \''.$metadatas.'\');">';
                    if($Pager->_currentPage == ($iIndex))
                        $szPages .= '<b>' . $iIndex . '</b></a> | ';
                    else
                        $szPages .= $iIndex . '</a> | ';
            ...
            if($Pager->_currentPage != $Pager->_lastPageText)
                $szPages .= '... <a style="cursor:pointer;" onClick="get_images('.$category.','.$Pager->_lastPageText.', \''.$action.'\', \''.$metadatas.'\');">Última </a>';
            $szPages .= "</p> ";
        }
        return $szPages;
    }
   ...
   function get_types()
    ...
    function get_permalinks_by_categoryID($categoryID, $filter=NULL, $_order_by='ORDER BY 1')
    {
        ...
        $sql= 'SELECT contents.title,contents.permalink,contents.created,contents.changed,contents.metadata,contents.starttime,contents.endtime FROM contents, contents_categories '.
    ...
    */
    ...
    function getOpinionAuthorsPermalinks($filter=NULL, $_order_by='ORDER BY 1')
    {
        $this->init($content_type);
        $items = array();
        $_where = '1=1  AND in_litter=0';
    ...
    function get_categoryName_by_contentId($contentId) {
           ...
           $rs = $GLOBALS['application']->conn->Execute($sql);
            ...
            if($rs!==false) {
                while(!$rs->EOF) {
                    $obj = new Content();
                    $obj->load($rs->fields);
                    $obj->content_type=$GLOBALS['application']->conn->GetOne('SELECT title FROM `content_types` WHERE pk_content_type = "'. $obj->fk_content_type.'"');
