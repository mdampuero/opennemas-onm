...
/**
    ...
    */
    ...
    function get_all_subcategories($category_id) {
        if( is_null($this->categories) ) {
            $sql = 'SELECT name,title FROM content_categories WHERE internal_category<>0 AND inmenu=1 AND
            ...
            $items = array ();
            while(!$rs->EOF) {
                $items[ $rs->fields['name'] ]=$rs->fields['title'];
                $rs->MoveNext();
            }
        }
        ...
        foreach($categories as $category) {
            if( ($category->internal_category) && ($category->inmenu == 1)
                ...
                $items[$category->name] = $category->title;
            }
        }                
 ...
 //Returns true if there is no contents in that category id
    ...
    */
    ...
    function getArraysMenu() {       
        ...
        $datos_cat = array();
        foreach( $fullcat as $prima) {
            if ( ($_REQUEST['category']!='home') && ($_REQUEST['category']!='todos') && ($prima->pk_content_category == $_REQUEST['category'] ) ) {
                $datos_cat[] = $prima;
            }
        }    
