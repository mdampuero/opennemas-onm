...
class PCUser_group {
    ...
    function __construct($id=NULL){
        $this->PCUser_group($id);
    }

    function create($data) {
    	//Se inserta el grupo
        $sql = "INSERT INTO pc_user_groups (`name`)
                    VALUES (?)";
    ...
    function read($id) {
        $sql = 'SELECT * FROM pc_user_groups WHERE pk_user_group = '.intval($id);
        ...
        $sql = 'SELECT pk_fk_privilege FROM pc_user_groups_privileges WHERE pk_fk_user_group = '.intval($id);
        $rs = $GLOBALS['application']->conn->Execute( $sql );
        if (!$rs) {
            $error_msg = $GLOBALS['application']->conn->ErrorMsg();
            $GLOBALS['application']->logger->debug('Error: '.$error_msg);
    ...
    function update($data) {
    	if(!is_null($data['id'])){
        	...
        	$sql = "UPDATE pc_user_groups SET `name`=?
    ...
    function delete($id) {
        $sql = 'DELETE FROM pc_user_groups WHERE pk_user_group='.intval($id);
    ...
    function get_user_groups(){
        ...
        $types = array();
        $sql = 'SELECT pk_user_group, name FROM pc_user_groups';
        $rs = $GLOBALS['application']->conn->Execute($sql);
        while(!$rs->EOF) {
    ...
    private function insert_privileges($data){
		$sql = "INSERT INTO pc_user_groups_privileges (`pk_fk_user_group`, `pk_fk_privilege`)
                    VALUES (?,?)";
    ...
    private function delete_privileges($id) {
        $sql = 'DELETE FROM pc_user_groups_privileges WHERE pk_fk_user_group='.intval($id);
        if($GLOBALS['application']->conn->Execute($sql)===false) {
            $error_msg = $GLOBALS['application']->conn->ErrorMsg();
            $GLOBALS['application']->logger->debug('Error: '.$error_msg);
            $GLOBALS['application']->errors[] = 'Error: '.$error_msg;
