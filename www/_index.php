...
if(($tpl->caching == 0) || !$tpl->is_cached('index.tpl', $cache_id)) { // (2)
    ...
    $articles = array();
    foreach($articles_home as $a) {
        // TODO: manage default mask
        if( is_null($a->mask) ) {
            $a->mask = 'frontpage_article';
        }
        
        $articles[ $a->{$placeholder} ][] = new ContentBox($a);
    }
    
    $wgt = new Widget('2010041220015200698');
    $wgt->mask = 'frontpage_widget_lateral';    
    $articles[ $wgt->{$placeholder} ][] = new ContentBox($wgt);
    
    $wgt = new Widget('2010041420050800771');
    $wgt->mask = 'frontpage_widget_lateral';
    $wgt->home_placeholder = 'content-long';
    $articles[ $wgt->{$placeholder} ][] = new ContentBox($wgt);
    
    
    
    $tpl->assign('grid_content', $grid->render($articles));
