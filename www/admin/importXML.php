...
require_once('core/pclave.class.php');
...
function createArticle($elementXML,$numCategories,$check='1')
{
  ...
  // $data['body'] = splitBodyInHtmlParagraph($elementXML->textoArticulo);
    ...
    if (($current_category == 'política') || ($current_category == 'polItica')|| ($current_category == 'politica')){        
            $current_category = 'polItica';
        } 
    $data['category'] = $ccm->get_id($current_category);
 
    //If the system does not recognize the category, send to unknown category
    if (empty($data['category'])) {
        if (($current_category == 'sociedade') || ($current_category == 'actualidade') || ($current_category == 'ciencia e tecnoloxia')){
            $current_category = 'sociedad';
        }else {
