...
define('PATH_FILES', realpath(dirname(__FILE__).'/../media/images/'));
...
$path = (isset($_POST['path']))? DialogExplorer::path2url(DialogExplorer::url2path($_POST['path'])): 	DialogExplorer::path2url(PATH_FILES."/".$cat);
...
function preview_in_opener(frm, campo_retorno) {
	...
	if(carac.ancho < carac.alto) {
		    var html_str = '<img src="<?=PATH_FILES?>/'+frm.campo1.value+'" border="0"  height="'+alti+'"  style="margin-left: 60px;" />';
		    opener.document.getElementById('preview_'+campo_retorno).innerHTML = html_str;
    }else{
		     var html_str = '<img src="<?=PATH_FILES?>/'+frm.campo1.value+'" border="0"  width="'+fijo+'"  style="margin-left: 10px;" />';
		    opener.document.getElementById('preview_'+campo_retorno).innerHTML = html_str;
    }
    
      if( /\.(swf)$/i.test(frm.campo1.value) ) {
           var html_str = '<object ><param name="movie" value="<?=URL_UPLOAD?>/'+frm.campo1.value+'"><embed src="<?=PATH_FILES?>/'+frm.campo1.value+'" width="300" ></embed></object>';
		    opener.document.getElementById('preview_'+campo_retorno).innerHTML = html_str;
      }
    ...
    if(frm.campo1.value != '') {
      if(campo_retorno != 'imgs' ) { //opinion    
		   if(campo_retorno == 'img1' ) {		     	       
					...
					if(opener.document.getElementById('eje1')){  //Ocultar ejemplo
		...
		<td>
	    	...
	    	//print_r($photos);
	    			   ...
	    			   foreach($photos as $ph) {
	    			     echo('<option de:clave="'.$ph->pk_photo.'" de:tam="'.$ph->size.'" de:fecha="'.date("d/m/Y",$ph->created).'" value="'.$ph->name.'" de:url="'.$path.'/'.$ph->path_file.'" >'.$ph->name.' - '.$ph->title.'</option>');
	    			   }
	    			   echo('</select>');
	    		?>
	    		<!-- 
