...
class DevelController extends Onm_Controller_Action
{
    ...
    {
        $queries = array();
        $total = $this->_getParam('total', 30);
        
        $this->conn->Execute('DELETE FROM `contents` WHERE `pk_content` IN (' . implode(',', range(0, $total)) . ')');
        $this->conn->Execute('DELETE FROM `articles` WHERE `pk_article` IN (' . implode(',', range(0, $total)) . ')');
        $this->conn->Execute('DELETE FROM `contents_pages` WHERE `pk_fk_content` IN (' . implode(',', range(0, $total)) . ')');
        ...
        $contents_pages = "INSERT INTO `contents_pages` (`pk_fk_page`, `pk_fk_content`, `pk_placeholder`, `pk_weight`, `mask`, `params`) VALUES ('%d', '%d', '%s', '%d', NULL, NULL);";
        ...
        $placeholders = array('content-left', 'content-center', 'content-right','content-long');
