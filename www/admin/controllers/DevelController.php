...
class DevelController extends Onm_Controller_Action
{
    ...
    {
        $queries = array();
        $total = $this->_getParam('total', 50);
        
        $this->conn->Execute('DELETE FROM `contents` WHERE `pk_content` IN (' . implode(',', range(40, $total)) . ')');
        $this->conn->Execute('DELETE FROM `articles` WHERE `pk_article` IN (' . implode(',', range(40, $total)) . ')');
        $this->conn->Execute('DELETE FROM `contents_pages` WHERE `pk_fk_content` IN (' . implode(',', range(40, $total)) . ')');
        ...
        $contents_pages = "INSERT INTO `contents_pages` (`pk_fk_page`, `pk_fk_content`, `pk_placeholder`, `pk_weight`, `mask`, `params`) VALUES ('%d', '%d', '%s', '%d', 'frontpage_article_lateral', NULL);";
        
        
        
        $page = new Page();
        $page = $page->getPageBySlug('galiza');
        
        $placeholders = array('content-center', 'content-right');
