                                        ...
                                        <div class="thumbnail"  onmouseover="document.getElementById('header1-{$smarty.section.n.index}').style.display='inline';" onmouseout="document.getElementById('header1-{$smarty.section.n.index}').style.display='none';"  {if  $smarty.server.PHP_SELF neq '/admin/mediamanager.php'} style="margin-bottom: 24px;"{/if}>
                                            {if $photo[n]->content_status eq 1 }
                                                <div id="header1-{$smarty.section.n.index}" style="display:none" class="thumbnail-file" z-index="1000">
                                                    ...
                                                    <a style="cursor:pointer;" onmouseover="Tip('<img src=\'{$MEDIA_IMG_PATH}{$photo[n]->path_file}{$photo[n]->name}\'>', SHADOW, true, ABOVE, true, WIDTH, {$photo[n]->width})" onmouseout="UnTip()" >
                                                ...
                                                <div class="table_div">
                                                    <div>
                                                        {if preg_match('/^swf$/i', $photo[n]->type_img)}
                                                            <object>
                                                                <param name="wmode" value="transparent"
                                                                       value="{$MEDIA_IMG_PATH}{$photo[n]->path_file}{$photo[n]->name}" />
                                                                <embed wmode="transparent"
                                                                       src="{$MEDIA_IMG_PATH}{$photo[n]->path_file}{$photo[n]->name}"
                                                        ...
                                                        {elseif preg_match('/^(jpeg|jpg|gif|png)$/i', $photo[n]->type_img)}
                                                            <img  onClick="javascript:$('selected_{$smarty.section.n.iteration}').click();" onmouseover="document.getElementById('header1-{$smarty.section.n.index}').style.display='inline';document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" onmouseout="document.getElementById('header1-{$smarty.section.n.index}').style.display='none';document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" src='{$MEDIA_IMG_PATH}{$photo[n]->path_file}140x100-{$photo[n]->name}' style='border:0px;'  maxWidth='140' maxHeight='100' />

                                                        {else}
                                                            <object  onClick="javascript:$('selected_{$smarty.section.n.iteration}').click();" onmouseover="document.getElementById('header1-{$smarty.section.n.index}').style.display='inline';document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" onmouseout="document.getElementById('header1-{$smarty.section.n.index}').style.display='none';document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" ><param name="movie" value="{$MEDIA_IMG_PATH}{$photo[n]->path_file}{$photo[n]->name}" /><embed src="{$MEDIA_IMG_PATH}{$photo[n]->path_file}{$photo[n]->name}" width="140" height="80" ></embed></object>
                                                        {/if}
                                                    </div>
                                                </div>
                                                ...
                                                <div style="display:inline" id="header2-{$smarty.section.n.index}" class="thumbnail-info" z-index="1000">
                                                    ...
                                                    {else}
                                                        <a onmouseover="Tip('{$smarty.const.MEDIA_IMG_PATH_URL}{$photo[n]->path_file}{$photo[n]->name}', ABOVE, true,OFFSETY, -30, WIDTH, 400, CLOSEBTN, true, CENTERMOUSE, true, FOLLOWMOUSE, false)">Ver URL</a>
                                                    {/if}
                                            ...
                                            {else}
                                                <img  onClick="javascript:$('selected_{$smarty.section.n.iteration}').click();" title="{$photo[n]->name}" alt="{$photo[n]->name}" src='{$MEDIA_IMG_PATH}/140x100-nodisp_img.jpg' style='border:0px;'  maxWidth='140' maxHeight='100' />
                                                ...
                                                <div style="display:inline" onmouseover="document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" onmouseout="document.getElementById('header2-{$smarty.section.n.index}').style.display='inline';" id="header2-{$smarty.section.n.index}" class="thumbnail-info" z-index="1000">
                                                      <input type="checkbox"  style="float:left;vertical-align:bottom;margin-bottom:1px;margin-top:0px;margin-right:1px;"  class="minput"  id="selected_{$smarty.section.n.iteration}" name="selected_fld[]" value="{$photo[n]->id}"  style="cursor:pointer;">
                                                        {$photo[n]->size}KB ({$photo[n]->width}x{$photo[n]->height})
                                                    <a href="#" onclick="javascript:confirmar('?action=delFile&amp;id={$photo[n]->pk_photo}&amp;basename={$photo[n]->name}&amp;path={$path}&amp;listmode=weeks&amp;category={$category}');" title="Eliminar fichero">
                                                        <img src="{php}echo($this->image_dir);{/php}iconos/eliminar.gif" border="0" align="absmiddle" />
                                                    </a>
