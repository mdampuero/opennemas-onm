	 ...
	 <div id="scroll-album">
		 <ul class="gallery_list" id="album_list">
				 ...
				 {if $oldphoto[n]->content_status eq 1 }				
					<li value="{$MEDIA_IMG_PATH_URL}{$oldphoto[n]->path_file}{$oldphoto[n]->name}" de:pk_photo="{$oldphoto[n]->pk_photo}"  id="f{$indi}-{$oldphoto[n]->pk_photo}">
					    <a class="album" onClick="show_image('img{$oldphoto[n]->pk_photo}', 'f{$indi}-{$oldphoto[n]->pk_photo}')" title="Mostrar imagen">
					     <img ondblclick="define_crop(this);"  style="{cssimagescale resolution=67 photo=$oldphoto[n]}" src="{$MEDIA_IMG_PATH_URL}{$oldphoto[n]->path_file}{$oldphoto[n]->name}"  class="draggable2" id="img{$oldphoto[n]->pk_photo}" de:pk_photo="{$oldphoto[n]->pk_photo}"  value="f{$indi}-{$oldphoto[n]->pk_photo}" name="{$oldphoto[n]->name}" de:mas="{$oldphoto[n]->name}"   de:path="{$oldphoto[n]->path_file}"  border="0" de:dimensions="{$oldphoto[n]->width} x {$oldphoto[n]->height} (px)" de:peso="{$oldphoto[n]->size}" de:created="{$oldphoto[n]->created}"  de:description="{$oldphoto[n]->description|escape:"html"}"  de:tags="{$oldphoto[n]->metadata}"   de:footer="{$oldphotos[n][2]|escape:"html"}"/><br>
					   </a></li>                                          
				   ...
				   <hr style=" margin:0;border-style: solid; color:#2f6d9d; border-width: 2px;"></h2>
                                ...
                                <tr><td colspan=2>
                                    ...
                                    <input type="hidden" id="img_des" value="" size="60">
                                     ...
                                     <img src="../media/images/default_img.jpg" id="image_view" name="imag_view" width=300 border="0" />
...
<tr>
    <td colspan="3">
           Recorte la imagen que se visualiza en portada.(250x250 px)

             <div id="testWrap">
                    <img src="../media/images/default_img.jpg" alt="test image" id="testImage"  width="300" height="208" />
            </div>
             <div id="previewArea"> {if $crop_exist} <img id="crop_img" src="../media/images/album/crops/{$album->id}.jpg" alt="Imagen en portada"  style="maxWidth:600px;maxHeight:400px;" /> {/if} </div>
             <div id="results">
...
<script type="text/javascript">
  ...
  function define_crop(element) {
              ...
              if(crop != null) {
                ...
                if(b[0]<250){alert('La foto escogida para portada no supera los 250px de ancho');
                }else{ if(c[0]<250){alert('La foto escogida para portada no supera los 250px de alto');}
                }
                cropcreate();
 }
